...
-----------------------------------------------------------------
create or replace function product_types_upsert(in par_product_type_id int, in par_name text, in par_description text) returns text as $$
  DECLARE
    loc_response text;
  BEGIN
    if par_product_type_id isnull then
...
-----------------------------------------------------------------
create or replace function product_types_get(in par_product_type_id int) returns setof product_types as $$
  BEGIN
    if par_product_type_id isnull then
      return query select * from product_types;
    else
      return query select * from product_types where product_type_id = par_product_type_id;
    end if;
  END;
$$ LANGUAGE 'plpgsql';
-----------------------------------------------------------------
...
-----------------------------------------------------------------
create or replace function categories_upsert(in par_category_id int, in par_name text, in par_description text, in par_image text) returns text as $$
  DECLARE
    loc_response text;
  BEGIN
    if par_category_id isnull then
...
-----------------------------------------------------------------
create or replace function categories_get(in par_category_id int) returns setof categories as $$
  BEGIN
    if par_category_id isnull then
      return query select * from categories;
    else
      return query select * from categories where category_id = par_category_id;
    end if;
  END;
$$ LANGUAGE 'plpgsql';
-----------------------------------------------------------------
...
-----------------------------------------------------------------
create or replace function product_attributes_upsert(in par_product_type_id int, in par_product_attribute_id int, in par_name text, in par_code text, in par_type text, in par_is_active boolean) returns text as $$
  DECLARE
    loc_response text;
  BEGIN
    if par_product_attribute_id isnull then
...
-----------------------------------------------------------------
create or replace function product_attributes_get(in par_product_type_id int, in par_product_attribute_id int) returns setof product_attributes as $$
  BEGIN
    if par_product_attribute_id isnull then
      return query select * from product_attributes where product_type_id = par_product_type_id;
    else
      return query select * from product_attributes where product_type_id = par_product_type_id and product_attribute_id = par_product_attribute_id;
    end if;
  END;
$$ LANGUAGE 'plpgsql';
-----------------------------------------------------------------
